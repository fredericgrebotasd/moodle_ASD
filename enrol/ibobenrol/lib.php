...
class enrol_ibobenrol_plugin extends enrol_plugin {
    ...
    public function get_newinstance_link($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/ibobenrol:config', $context)) {
            return null;
        }
        // Multiple instances supported - different roles with different password.
        return new moodle_url('/enrol/ibobenrol/edit.php', array('courseid' => $courseid));
    }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        if ($access) {
            ...
            print_r($configbadges);

            foreach ($configbadges as $badgeid) {

                $badge = $DB->get_record('local_ibob_badges', array('id' => $badgeid), '*', MUST_EXIST);
...
//                    $badgeout .= html_writer::tag('span', ' ('.get_string('issuedatetext', 'enrol_ibobenrol').
//                        date(get_string('issuedateformat', 'enrol_ibobenrol') , $issuedate).')',
...
//                }
    ...
    public function check_required_badges($userid, $badges) {
        ...
        foreach ($badges as $badgeid) {
            if ($record = $DB->get_record('local_ibob_badge_issued', array('badgeid' => $badgeid, 'userid' => $userid))) {
//            if ($record = $DB->get_record('badge_issued', array('badgeid' => $badgeid, 'userid' => $userid))) {
                if (!$record->expirationdate or $record->expirationdate >= time()) {
                    $access = true;
                } else {
                    $access = false;
                    break;
